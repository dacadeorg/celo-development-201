...
Now we can compile the contract, with following command:

```sh
npx hardhat compile
```

After a succeful compilation you should the following output:
...
### 3.2 Tests
Let's add some tests for our contract. Inside the `tests` folder, we delete the `contracts-test.js` file and create a new one called `nft-test.js`. Add the following code to the file:
```js
...
```
In this testing file, we test 


We explain the tests in this file in detail in the [NFT Contract development with Hardhat]() learning module.